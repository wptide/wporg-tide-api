...
class Test_Plugin_Base extends \WP_UnitTestCase {
	...
	public function test_locate_plugin() {
		...
		$this->assertEquals( 'wporg-tide-api', $location['dir_basename'] );
		$this->assertContains( 'plugins/wporg-tide-api', $location['dir_path'] );
		$this->assertContains( 'plugins/wporg-tide-api', $location['dir_url'] );
	}
	...
	public function test_relative_path() {
		$this->assertEquals( 'plugins/wporg-tide-api', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/plugins/wporg-tide-api', 'wp-content', '/' ) );
		$this->assertEquals( 'themes/twentysixteen/plugins/wporg-tide-api', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/themes/twentysixteen/plugins/wporg-tide-api', 'wp-content', '/' ) );
	}
	...
	public function test_trigger_warning() {
		...
		set_error_handler( function ( $errno, $errstr ) use ( $obj ) {
			$obj->assertEquals( 'WPOrg_Tide_API\Plugin: Param is 0!', $errstr );
	...
	public function test_add_doc_hooks_error() {
		$mock = $this->getMockBuilder( 'WPOrg_Tide_API\Plugin' )
